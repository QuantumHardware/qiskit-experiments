...
class TestCurveFitting(QiskitExperimentsTestCase):
    ...
    def test_process_curve_data(self):
        ...
        xdata, ydata, _ = process_curve_data(
    ...
    def test_curve_fit(self):
        ...
        with self.assertWarns(DeprecationWarning):
            sol = curve_fit(
                self.objective0, xdata, ydata, p0, sigma=sigma, bounds=bounds
            )
    ...
    def test_mean_xy_data(self):
        ...
        self.assertTrue(
            np.allclose(expected_y_sigma, y_sigma),
            msg=f"{y_sigma} != {expected_y_sigma}",
        )
        ...
        expected_y_sigma = np.array(
            [0.85714286, 2.57610543, 5.97927455, 10.0, 6.17470935]
        )
        self.assertTrue(
            np.allclose(expected_x_mean, x_mean), msg=f"{x_mean} != {expected_x_mean}"
        )
        ...
        expected_y_sigma = np.array(
            [1.27475488, 3.26190129, 6.02079729, 10.0, 6.46166282]
        )
        ...
        self.assertTrue(
            np.allclose(expected_y_shots, y_shots),
            msg=f"{y_shots} != {expected_y_shots}",
        )

        x = np.array([1, 1, 1, 1, 2, 2, 2, 2])
        y = np.array([2, 6, 100, 200, 17, 50, 60, 70])
        series = np.array([0, 0, 1, 1, 0, 1, 1, 1])
        series, x_mean, y_mean, y_sigma, _ = multi_mean_xy_data(
            series, x, y, method="sample"
        )
        expected_x_mean = np.array([1, 2, 1, 2])
        expected_y_mean = np.array([4, 17, 150, 60])
        expected_y_sigma = np.sqrt(
            np.array([4.0 / 2, 0.0 / 1, 2500.0 / 2, 66.66666667 / 3])
        )
